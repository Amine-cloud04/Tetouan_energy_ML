...
This project successfully deploys the optimized K-Nearest Neighbors (KNN) Regressor model into an interactive web application built with Streamlit. The deployment strategy ensures the model's high accuracy , achieved through advanced data preprocessing and hyperparameter tuning, is maintained in a production environment. The application utilizes the saved model object (knn_final_model.joblib), along with the necessary feature scaler and target scaler to handle real-time user inputs and correctly inverse-transform the predictions back into meaningful power consumption units. This setup allows any user to easily input key weather and temporal variables to receive immediate, highly accurate forecasts for total city power consumption.
...
4. Book: Hands On Machine Learning book (Aurelien Geron) 
5. Streamlit
